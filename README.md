...
- A portable Dev Bridge protocol, capability negotiation, revision lifecycle, typed FakeADB transcript runner, authenticated loopback hello server and safe reverse coordinator. The coordinator permits only one ready transport to map to the fixed Android port `49321`. The separate `lyra-adb` adapter accepts an explicit executable path and emits fixed no-shell argv, but Studio does not invoke it yet.
