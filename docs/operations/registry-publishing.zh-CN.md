...
每个仓库只需把 Pages source 设置为 **GitHub Actions** 一次，然后将私钥 seed 写入受保护的 Actions Secret，并保持两个官方身份变量（也可以使用 workflow 默认值）：
...
`Publish Theme Registry` workflow 在 `v*` tag 或手动 dispatch 时运行。它会先检查 Secret 和官方身份变量，再执行源码、许可证和可复现性门禁，签名目录与 Pack checksum，校验完整站点并部署以下文件：`registry-v1.json`、`registry-v1.sig`、`public-key.txt` 及版本化 Pack。

workflow 会拒绝与 Android 官方信任常量不一致的 `LYRA_REGISTRY_ID` 或 `LYRA_REGISTRY_KEY_ID`，防止将 Android 永远拒绝的“已签名目录”发布到 Pages。轮换 key 时必须同时评审 Android 常量和 workflow 门禁。
...
2. 在同一份评审变更中更新 GitHub protected Secret、workflow 官方身份门禁、`LYRA_REGISTRY_ID`/`LYRA_REGISTRY_KEY_ID` variables 和 Android 信任常量。
