...
Enable Pages with the **GitHub Actions** source once per repository. Then add the private seed as a protected Actions secret and keep both official identity variables (or use the workflow defaults):
...
gh secret set LYRA_REGISTRY_PRIVATE_KEY_BASE64 \
  ...
  < lyra-registry-private-key.base64
gh variable set LYRA_REGISTRY_ID \
  --repo anpplex/lyra-effects-studio \
  --body org.lyra.effects.official
...
The workflow rejects a configured `LYRA_REGISTRY_ID` or `LYRA_REGISTRY_KEY_ID` that differs from the Android official trust constants. This prevents a signed but unusable catalog from reaching Pages; coordinated key rotation must update the Android constants and this preflight together.
...
2. Update the protected GitHub secret, the workflow's official-identity preflight, `LYRA_REGISTRY_ID`/`LYRA_REGISTRY_KEY_ID` variables, and the Android trust constants in one reviewed change.
