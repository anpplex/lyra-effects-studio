...
- `lyra-adb` owns the explicit-path, fixed-argv, no-shell `AdbClient` process adapter. It depends on `lyra-device`; Tauri constructs it only inside an explicit user action: `list_devices` for readiness, or the typed coordinator's fixed reverse/remove operations for one owned mapping.
...
- `src-tauri` exposes narrowly scoped project commands, local Dev Bridge lifecycle commands, a private-path ADB preflight surface and three no-argument mapping actions to the frontend. It owns desktop lifetime, not HTTP protocol, validation or Registry trust decisions.
...
M3 adds the portable device domain, an authenticated IPv4-loopback hello server, a narrow Tauri lifecycle boundary, a fake-first reverse coordinator, an explicit ADB process adapter, a user-gated ADB preflight and one explicit reverse-mapping flow. `lyra-dev-server` owns real TCP protocol behavior; `src-tauri` starts or gracefully stops one instance and retains provisioning material privately. `lyra-device` selects exactly one ready transport and maps a caller-supplied loopback port to the fixed Android Dev Bridge port `49321` through injected `AdbClient`. A Rust-owned native picker supplies the preflight executable path; `src-tauri` retains its canonical regular-file path only in memory. **Check devices** calls only `list_devices`; a separately clicked **Enable mapping** derives the listener port privately and invokes the existing coordinator, while **Remove mapping** (or explicit bridge stop) invokes only its retained typed cleanup. The Studio frontend receives bridge state, `{ configured, readiness }` preflight and `{ readiness }` mapping projections; it never receives an executable path, serial, raw output, bearer, endpoint URL/port or server session ID. There is no automatic discovery or mapping, app-exit cleanup, background retry, Pack push or Android change. JSON fixtures continue to drive hello parsing and strict FakeADB transcripts; arbitrary shell commands remain absent from the domain API.
